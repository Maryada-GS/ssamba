...
AMBAModel (Mamba-based) for audio processing tasks.
...
class AMBAModel(nn.Module):
    def __init__(
        ...
        if pretrain_stage == True:
            ...
            self.cpredlayer = nn.Sequential(
                nn.Linear(
                    self.original_embedding_dim, self.original_embedding_dim
                ),
                nn.ReLU(),
        ...
        # use a pretrained models for finetuning
            ...
            print(
                "now load a SSL pretrained models from "
            ...
            audio_model = AMBAModel(
                ...
                vision_mamba_config=combined_vision_mamba_config,
            )
            ...
            p_f_dim, p_t_dim = (
                audio_model.module.p_f_dim,
                audio_model.module.p_t_dim,
            )
    ...
    def get_shape(
        self, fstride, tstride, input_fdim, input_tdim, fshape, tshape
    ):
    ...
    # only used for urban8k, not tested so commenting it out
    ...
    #         fused_add_norm_fn = (
    #             rms_norm_fn
    #             if isinstance(self.v.norm_f, RMSNorm)
    #             else layer_norm_fn
    #         )
    ...
    def mpc(self, x, mask_patch, cluster, show_mask=False):
        ...
        for i in range(B):
            ...
            if cluster == True:
                ...
                mask_index[i] = self.gen_maskid_patch(
                    self.num_patches, mask_patch
                )
            else:
                # use this if you are masking frame, i.e., 128*2 patches
                mask_index[i] = self.gen_maskid_frame(
                    self.num_patches, mask_patch
                )
        ...
        pred = torch.empty(
            (B, mask_patch, 256), device=x.device
        ).float()  # e.g. size 12*100*768
        for i in range(B):
            ...
            pred[i] = self.cpredlayer(
                x[i, mask_index[i] + self.cls_token_num, :]
            )
        ...
        for i in np.arange(0, B):
            ...
            total = torch.mm(
                encode_samples[i], torch.transpose(pred[i], 0, 1)
            )  # e.g. size 100*100
            correct += torch.sum(
                torch.eq(
                    torch.argmax(self.softmax(total), dim=0),
                    torch.arange(0, mask_patch, device=x.device),
                )
            )  # correct is a tensor
            nce += torch.sum(
                torch.diag(self.lsoftmax(total))
            )  # nce is a tensor
        ...
        if show_mask == False:
            ...
            if B > 1:
                raise Exception(
                    "Currently only support single spectrogram probing test."
                )

            self.mask_correct = torch.nn.Parameter(
                torch.arange(0, mask_patch), requires_grad=False
            )
            ...
            for i in range(B):
                ...
                pred[i, mask_index[i], :] = (
                    torch.tensor(result)
                    .reshape(mask_patch, 1)
                    .expand(mask_patch, 256)
                )
    ...
    def mpg(self, input, mask_patch, cluster):
        ...
        pred = torch.empty(
            (B, mask_patch, self.fshape * self.tshape), device=x.device
        ).float()  # e.g. size 12*100*256
        target = torch.empty(
            (B, mask_patch, self.fshape * self.tshape), device=x.device
        ).float()  # e.g. size 12*100*256
    ...
    def forward(self, x, task, cluster=True, mask_patch=400):
        ...
        # pretraining, masked patch reconstruction (generative objective)
            ...
            return self.mpc(
                x, mask_patch=mask_patch, cluster=cluster, show_mask=True
            )
        else:
