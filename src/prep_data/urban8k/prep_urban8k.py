...
def concatenate_audios(fold, audio_base_path, desired_length, segment_duration):
    ...
    for index, row in fold_metadata.iterrows():
        ...
        if len(audio_segment) % segment_duration != 0:
            pad_length = segment_duration - (
                len(audio_segment) % segment_duration
            )
    ...
    if current_length > 0:
        ...
        current_labels.extend(
            [current_labels[-1]] * (pad_length // segment_duration)
        )
        concatenated_segments.append(current_segment[:desired_length])
        concatenated_labels.append(
            current_labels[: desired_length // segment_duration]
...
for fold in range(1, 11):
    ...
    concatenated_segments, concatenated_labels = concatenate_audios(
        fold, audio_base_path, desired_length, segment_duration
    )
    ...
    for i, (segment, labels) in enumerate(
        zip(concatenated_segments, concatenated_labels)
    ):
    ...
    metadata_df.to_csv(
        os.path.join(output_fold_path, "concatenated_metadata.csv"), index=False
    )
