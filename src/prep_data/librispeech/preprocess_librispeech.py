...
def process_audio_file(
    input_filepath, output_directory, processed_files_log, error_files_log
):
    try:
        ...
        if (
            waveform.shape != original_waveform_shape
            or sample_rate != original_sample_rate
        ):
...
def preprocess_librispeech(input_directories, output_base_directory):
    ...
    for input_directory in input_directories:
        output_directory = os.path.join(
            output_base_directory, os.path.basename(input_directory)
        )
    ...
    print("Error files logged in error_files_log.txt")
