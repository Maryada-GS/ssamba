...
def trainmask(audio_model, train_loader, test_loader, args):
    ...
    def _save_progress():
        progress.append(
            [epoch, global_step, best_epoch, time.time() - start_time]
        )
    ...
    print(
        "Total parameter number is : {:.9f} million".format(
            sum(p.numel() for p in audio_model.parameters()) / 1e6
        )
    )
    print(
        "Total trainable parameter number is : {:.9f} million".format(
            sum(p.numel() for p in audio_trainables) / 1e6
        )
    )
    trainables = audio_trainables
    optimizer = torch.optim.AdamW(
        trainables, args.lr, weight_decay=5e-8, betas=(0.95, 0.999)
    )
    ...
    while epoch < args.n_epochs + 1:
        ...
        torch.save(
            ...
            "%s/models/audio_model.%d.pth" % (exp_dir, global_step + 1),
        )
        ...
        for i, (audio_input, _) in enumerate(train_loader):
            ...
            if global_step % epoch_iteration == 0:
                ...
                acc_eval, nce_eval = validatemask(
                    audio_model, test_loader, args, equ_epoch
                )
                ...
                torch.save(
                    audio_model.state_dict(),
                    "%s/models/audio_model.%d.pth" % (exp_dir, equ_epoch),
                )
                if len(train_loader.dataset) > 2e5:
                    torch.save(
                        optimizer.state_dict(),
                        "%s/models/optim_state.pth" % (exp_dir),
                    )
                ...
                print(
                    "# {:d}, step {:d}-{:d}, lr: {:e}".format(
                        equ_epoch,
                        global_step - epoch_iteration,
                        global_step,
                        optimizer.param_groups[0]["lr"],
                    )
                )
                ...
                print(
                    "# {:d}, step {:d}-{:d}, training time: {:.3f}".format(
                        equ_epoch,
                        global_step - epoch_iteration,
                        global_step,
    ...
    with torch.no_grad():
        for i, (audio_input, _) in enumerate(val_loader):
            ...
            if args.task == "pretrain_mpc":
                ...
                acc, _ = audio_model(
                    audio_input, "pretrain_mpc", mask_patch=400, cluster=cluster
                )
                mse = audio_model(
                    audio_input, "pretrain_mpg", mask_patch=400, cluster=cluster
                )
