...
parser.add_argument(
    "--data-train", type=str, default=None, help="training data json"
)
parser.add_argument(
    "--data-val", type=str, default=None, help="validation data json"
)
parser.add_argument(
    "--data-eval", type=str, default=None, help="evaluation data json"
)
parser.add_argument(
    "--label-csv", type=str, default="", help="csv with class labels"
)
parser.add_argument(
    "--n_class", type=int, default=527, help="number of classes"
)
...
parser.add_argument(
    "--target_length", type=int, help="the input length in frames"
)
parser.add_argument(
    "--num_mel_bins", type=int, default=128, help="number of input mel bins"
)
...
parser.add_argument(
    "--adaptschedule",
    help="if use adaptive scheduler ",
    type=ast.literal_eval,
    default="False",
)
...
parser.add_argument(
    "--save_model", help="save the models or not", type=ast.literal_eval
)
...
parser.add_argument(
    "--fshape", type=int, help="shape of patch on the frequency dimension"
)
parser.add_argument(
    "--tshape", type=int, help="shape of patch on the time dimension"
)
parser.add_argument(
    "--model_size", help="the size of AST models", type=str, default="base384"
)
...
parser.add_argument(
    "--mask_patch",
    help="how many patches to mask (used only for ssl pretraining)",
    type=int,
    default=400,
)
parser.add_argument(
    "--cluster_factor", type=int, default=3, help="mask clutering factor"
)
parser.add_argument(
    "--epoch_iter",
    type=int,
    default=2000,
    help="for pretraining, how many iterations to verify and save models",
)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="the ssl pretrained models path",
)
parser.add_argument(
    "--head_lr",
    type=int,
    default=1,
    help="the factor of mlp-head_lr/lr, used in some fine-tuning experiments only",
)
parser.add_argument(
    "--noise", help="if augment noise in finetuning", type=ast.literal_eval
)
parser.add_argument(
    "--metrics",
    type=str,
...
parser.add_argument(
    "--rms_norm", type=str, choices=["true", "false"], default="true"
)
parser.add_argument(
    "--residual_in_fp32", type=str, choices=["true", "false"], default="true"
)
parser.add_argument(
    "--fused_add_norm", type=str, choices=["true", "false"], default="true"
)
parser.add_argument(
    "--if_rope", type=str, choices=["true", "false"], default="false"
)
parser.add_argument(
    "--if_rope_residual", type=str, choices=["true", "false"], default="false"
)
...
parser.add_argument(
    "--if_bidirectional", type=str, choices=["true", "false"], default="true"
)
parser.add_argument("--final_pool_type", type=str, default="none")
parser.add_argument(
    "--if_abs_pos_embed", type=str, choices=["true", "false"], default="false"
)
parser.add_argument(
    "--if_bimamba", type=str, choices=["true", "false"], default="true"
)
parser.add_argument(
    "--if_cls_token", type=str, choices=["true", "false"], default="false"
)
parser.add_argument(
    "--if_devide_out", type=str, choices=["true", "false"], default="false"
)
parser.add_argument(
...
dataset = dataloader.AudioDataset(
    args.data_train, label_csv=args.label_csv, audio_conf=audio_conf
)
...
if "pretrain" in args.task:
    ...
    if cluster == True:
        print(
            "The num_mel_bins {:d} and fshape {:d} are different, not masking a typical time frame, using cluster masking.".format(
                args.num_mel_bins, args.fshape
            )
        )
    else:
        print(
            "The num_mel_bins {:d} and fshape {:d} are same, masking a typical time frame, not using cluster masking.".format(
                args.num_mel_bins, args.fshape
            )
        )
...
if "pretrain" not in args.task:
    ...
    print(
        "Now starting self-supervised pretraining for {:d} epochs".format(
            args.n_epochs
        )
    )
