...
def sampled_margin_rank_loss(
    ...
    for i in range(n):
        ...
        anchorsim = matchmapSim(
            computeMatchmap(image_outputs[i], audio_outputs[i][:, 0:nF]),
            simtype,
        )
        Iimpsim = matchmapSim(
            computeMatchmap(
                image_outputs[I_imp_ind], audio_outputs[i][:, 0:nF]
            ),
            simtype,
        )
        Aimpsim = matchmapSim(
            computeMatchmap(
                image_outputs[i], audio_outputs[A_imp_ind][:, 0:nFimp]
            ),
            simtype,
        )
...
def compute_pooldot_similarity_matrix(image_outputs, audio_outputs, nframes):
    ...
    pooled_audio_outputs = (
        torch.cat(pooled_audio_outputs_list).squeeze(3).squeeze(2)
    )
