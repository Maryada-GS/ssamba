...
print(
    "Now download and process speechcommands dataset, it will take a few moments..."
)
...
if os.path.exists("./data/speech_commands_v0.01") == False:
    ...
    os.system(
        "tar -xzvf ./data/speech_commands_v0.01.tar.gz -C ./data/speech_commands_v0.01"
    )
...
label_set = np.loadtxt(
    "./data/speechcommands_class_labels_indices.csv", delimiter=",", dtype="str"
)
label_map = {}
...
if os.path.exists("./data/datafiles") == False:
    ...
    for split in ["testing", "validation", "train"]:
        ...
        for file in filelist:
            ...
            cur_dict = {
                "wav": cur_path,
                "labels": "/m/spcmd" + cur_label.zfill(2),
            }
            wav_list.append(cur_dict)
        if split == "train":
            with open(
                "./data/datafiles/speechcommand_train_data.json", "w"
            ) as f:
                json.dump({"data": wav_list}, f, indent=1)
        if split == "testing":
            with open(
                "./data/datafiles/speechcommand_eval_data.json", "w"
            ) as f:
                json.dump({"data": wav_list}, f, indent=1)
        if split == "validation":
            with open(
                "./data/datafiles/speechcommand_valid_data.json", "w"
            ) as f:
                json.dump({"data": wav_list}, f, indent=1)
        print(
            split
            + " data processing finished, total {:d} samples".format(
                len(wav_list)
            )
        )
