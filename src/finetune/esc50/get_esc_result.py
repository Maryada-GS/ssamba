...
parser.add_argument(
    "--exp_path", type=str, default="", help="the root path of the experiment"
)
...
if __name__ == "__main__":
    ...
    for fold in range(1, 6):
        result = np.loadtxt(
            args.exp_path + "/fold" + str(fold) + "/result.csv", delimiter=","
        )
        if fold == 1:
    ...
    np.savetxt(
        args.exp_path + "/best_result.csv", result[best_epoch, :], delimiter=","
    )
