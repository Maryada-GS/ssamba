...
label_set = np.loadtxt(
    "./data/esc_class_labels_indices.csv", delimiter=",", dtype="str"
)
label_map = {}
...
for fold in [1, 2, 3, 4, 5]:
    ...
    with open(
        "./data/datafiles/esc_train_data_" + str(fold) + ".json", "w"
    ) as f:
        json.dump({"data": train_wav_list}, f, indent=1)

    with open(
        "./data/datafiles/esc_eval_data_" + str(fold) + ".json", "w"
    ) as f:
