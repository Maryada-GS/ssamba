...
def train(audio_model, train_loader, test_loader, args):
    ...
    mlp_params = list(
        filter(
            lambda kv: kv[0] in mlp_list, audio_model.module.named_parameters()
        )
    )
    base_params = list(
        filter(
            lambda kv: kv[0] not in mlp_list,
            audio_model.module.named_parameters(),
        )
    )
    ...
    print(
        "Total mlp parameter number is : {:.3f} million".format(
            sum(p.numel() for p in mlp_params) / 1e6
        )
    )
    print(
        "Total base parameter number is : {:.3f} million".format(
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (audio_input, labels) in enumerate(train_loader):
            ...
            if isinstance(loss_fn, torch.nn.CrossEntropyLoss):
                loss = loss_fn(
                    audio_output, torch.argmax(labels.long(), axis=1)
                )
            else:
            ...
            if print_step and global_step != 0:
                ...
                if np.isnan(loss_meter.avg):
                    ...
                    torch.save(
                        ...
                        "%s/models/nan_audio_model.pth" % (exp_dir),
                    )
                    torch.save(
                        optimizer.state_dict(),
                        "%s/models/nan_optim_state.pth" % (exp_dir),
                    )
                    with open(exp_dir + "/audio_input.npy", "wb") as f:
                        np.save(f, audio_input.cpu().detach().numpy())
                    np.savetxt(
                        exp_dir + "/audio_output.csv",
                        audio_output.cpu().detach().numpy(),
                        delimiter=",",
                    )
                    np.savetxt(
                        exp_dir + "/labels.csv",
        ...
        middle_ps = [
            stat["precisions"][int(len(stat["precisions"]) / 2)]
            for stat in stats
        ]
        middle_rs = [
            stat["recalls"][int(len(stat["recalls"]) / 2)] for stat in stats
        ]
        ...
        if best_epoch == epoch:
            ...
            torch.save(
                optimizer.state_dict(),
                "%s/models/best_optim_state.pth" % (exp_dir),
            )
        ...
        print(
            "Epoch-{0} lr: {1}".format(epoch, optimizer.param_groups[1]["lr"])
        )
        ...
        print(
            "epoch {:d} training time: {:.3f}".format(
    ...
    if args.wa == True:
        stats = validate_wa(
            audio_model, test_loader, args, args.wa_start, args.wa_end
        )
        mAP = np.mean([stat["AP"] for stat in stats])
        mAUC = np.mean([stat["auc"] for stat in stats])
        middle_ps = [
...
def validate(audio_model, val_loader, args, epoch):
    ...
    with torch.no_grad():
        ...
        if os.path.exists(exp_dir + "/predictions") == False:
            ...
            np.savetxt(
                exp_dir + "/predictions/target.csv", target, delimiter=","
            )
        np.savetxt(
...
def validate_ensemble(args, epoch):
    ...
    if epoch == 1:
        ...
        os.remove(
            exp_dir + "/predictions/predictions_" + str(epoch - 1) + ".csv"
        )

    cum_predictions = cum_predictions / epoch
    np.savetxt(
        exp_dir + "/predictions/cum_predictions.csv",
        cum_predictions,
        delimiter=",",
    )
...
def validate_wa(audio_model, val_loader, args, start_epoch, end_epoch):
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        sdB = torch.load(
            exp_dir + "/models/audio_model." + str(epoch) + ".pth",
            map_location=device,
        )
        for key in sdA:
